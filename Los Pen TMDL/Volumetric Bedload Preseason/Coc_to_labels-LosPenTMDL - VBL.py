...
## Set directory or path name of where the Chain of Custody is:
maindir = "C:/Users/alex.messina/Documents/GitHub/CoC-labels/Los Pen TMDL/Volumetric Bedload Preseason/"

## Input the name of the excel file that is the Chain of Custody form
Coc_Excel_File = 'VBL_TMDL_COC.xlsx'
...
import os
if 'git_template.docx' in os.listdir(maindir) == False:
    import requests
...
for row in merge_fields.iterrows():
    row_list.append({'NumberOfBottles':row[1]['NumberOfBottles'],'Container':row[1]['Container'],'ProjectName':row[1]['ProjectName'],'AnalysisSuite':row[1]['AnalysisSuite'],'SampleID':row[1]['SampleID'],'BottleNumber':row[1]['BottleNumber'],'Preservative':str(row[1]['Preservative'])})
